...
def count_mutations_per_gene_and_effect(snv, id_col, gene_col, effect_col):
    ...
    """
    counts = (
        snv[[id_col, gene_col, effect_col]]
        .drop_duplicates()
...
def compute_mutation_entropy(df_gene_effect):
    ...
    start, end = (
        int(df_gene_effect["Start"].values[0]),
        int(df_gene_effect["End"].values[0]),
    )
...
def compute_mutation_entropy_per_gene_and_effect(snv, annot, gene_col, effect_col):
    ...
    df = pd.merge(
        snv,
        annot[["Gene"] + cols_oi].rename(columns={"Gene": gene_col}),
        how="left",
        on="Hugo_Symbol",
    )
    ...
    entropies = (
        df.groupby([gene_col, effect_col]).apply(compute_mutation_entropy).reset_index()
    )
    entropies["min_entropy"] = -(1 / entropies["length"]) * np.log2(
        1 / entropies["length"]
    )
    entropies["max_entropy"] = -np.log2(
        1 / entropies["length"]
    )  # -len*(1/len)*log2(1/len)
...
def main():
    ...
    mut_entropy = compute_mutation_entropy_per_gene_and_effect(
        snv, annot, gene_col, effect_col
    )
